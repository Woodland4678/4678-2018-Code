...
#define SHOULDERLOWENCODER	-6382.0f //was -8371
#define	SHOULDERHIGENCODER	2743.0f	//was 3413

#define ELBOWLOWLIMIT		-159.2f
#define ELBOWHIGHLIMIT		164.0f
#define ELBOWLOWENCODER		2757.0f //Update this one
#define	ELBOWHIGENCODER		-11237.0f //Update this one

#define WRISTLOWLIMIT		-120.0f
#define WRISTHIGHLIMIT		120.0f
#define WRISTLOWENCODER		-1743.0f //Update this one
#define	WRISTHIGENCODER		4553.0f //Update this one
...
ManipulatorArm::ManipulatorArm() : frc::Subsystem("ManipulatorArm") {
	...
	do {
		...
		shAngCnt++;
	}while((std::abs(pulse - enc) > 100)&&(shAngCnt < 5));
	shoulderSeg.posX = 0;
	shoulderSeg.posY = 0;
    ...
    //Pick up cube
    positions[5][0] = 107;    //e:-1745   r:93
    positions[5][1] = -35;   //e:2478    r:-154
    positions[5][2] = -52;   //e:754     r:-28
    ...
    positions[11][1] = -14;  //e:2479    r:-154 SDW -14 (was-17)
...
bool ManipulatorArm::moveTo(int pos, double addShTime, double addElTime)
	...
	switch(moveCase)
		...
		case 4:
			...
			pickupCount++;
			//Pickup special case
			//Move the elbow and wrist to their positions
			if(pickupCount == 1)
				{
				setElbowAbsAngle(positions[5][1] + 20);
				setWristAbsAngle(positions[5][2]);
				}
			//Lets move straight down until we detect a cube in the claw
			if(pickupCount2 < 5)
				{
				double ang = positions[11][0] - (pickupCount * 1); // was 0.5
				setShoulderAbsAngle(ang);
				}
			if(checkForCube())
				pickupCount2++;
			else
		...
		case 5:
			...
			if(pickupCount > 20)//Wait a moment to ensure the claw clamped down
				{
				pickupCount2++;
...
bool ManipulatorArm::calibrate()
	{
	static int mainCase = 0, shCase = 0, elCase = 0, wrCase = 0;
	switch(mainCase)
		{
		case 0:
			shCase = 0;
		...
		case 1:
			switch(shCase)
				...
				case 1:
					//Check speed
					frc::SmartDashboard::PutNumber("Shoulder Speed", shoulder->GetActiveTrajectoryVelocity());
					frc::SmartDashboard::PutNumber("Shoulder Speed2", shoulder->GetSensorCollection().GetQuadratureVelocity());
					if (shoulder->GetSensorCollection().GetQuadratureVelocity() < 1)
						shCase = 2;
					break;
				case 2:
					//Angle is 127
					//Find the intercept
					shoulder->GetSensorCollection().SetQuadraturePosition(2808,0);
					//shoulderSeg.convIntercept = 127 - getShoulderAngular() * shoulderSeg.convSlope;
					frc::SmartDashboard::PutNumber("Shoulder New Int", getShoulderAngular());
					updateArm();
					shCase = 3;
					//mainCase = 2;
					break;
		...
		case 2:
			switch(elCase)
				...
				case 1:
					//Check speed
					if (elbow->GetSensorCollection().GetQuadratureVelocity() < 1)
						elCase = 2;
					break;
				case 2:
					//Angle is -159.2
					//Find the intercept
					elbowSeg.convIntercept = (-161.5) - getElbowAngular() * elbowSeg.convSlope;
					frc::SmartDashboard::PutNumber("Elbow New Int", elbowSeg.convIntercept);
					elCase = 3;
					break;
				}
			switch(wrCase)
				...
				case 1:
					//Check speed
					if (wrist->GetSensorCollection().GetQuadratureVelocity() < 1)
						wrCase = 2;
					break;
				case 2:
					//Angle is -120
					//Find the intercept
					wristSeg.convIntercept = -120 - getWristAngular() * wristSeg.convSlope;
					frc::SmartDashboard::PutNumber("Wrist New Int", wristSeg.convIntercept);
					wrCase = 3;
					break;
				}
...
int ManipulatorArm::getShoulderAngular()
	{
	frc::SmartDashboard::PutNumber("Shoulder En", shoulder->GetSelectedSensorPosition(0));
...
void ManipulatorArm::updateShoulder()
	...
	frc::SmartDashboard::PutNumber("Shoulder Encoder", shoulder->GetSensorCollection().GetQuadraturePosition());
	frc::SmartDashboard::PutNumber("Shoulder Encoder Abs", shoulder->GetSensorCollection().GetPulseWidthPosition());
	//Get angles
	...
	frc::SmartDashboard::PutNumber("Shoulder Slope", shoulderSeg.convSlope);
	frc::SmartDashboard::PutNumber("Shoulder Intercept", shoulderSeg.convIntercept);
...
void ManipulatorArm::updateElbow()
	...
	frc::SmartDashboard::PutNumber("Elbow Slope", elbowSeg.convSlope);
	frc::SmartDashboard::PutNumber("Elbow Intercept", elbowSeg.convIntercept);
	}
void ManipulatorArm::updateWrist()
	...
	frc::SmartDashboard::PutNumber("Wrist Slope", wristSeg.convSlope);
	frc::SmartDashboard::PutNumber("Wrist Intercept", wristSeg.convIntercept);
	}	
