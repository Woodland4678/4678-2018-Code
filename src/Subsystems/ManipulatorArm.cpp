...
ManipulatorArm::ManipulatorArm() : frc::Subsystem("ManipulatorArm") {
    ...
    positions[5][1] = -42;//-22;   //e:2478    r:-154
...
void ManipulatorArm::moveWrist(double joyValue)
	{
	double ang = wristSeg.setAbsAngle + (joyValue * 1.0 );
	if(ang < -80)
		ang = -80;
	if (ang > 143)
		ang = 143;
...
/*
	tarX += FINECONTROLBOXSIZE * joyX; // joyX, joyY in range -1.0 to +1.0
	...
	double dirX = tarX - wristSeg.setPosX;
	double dirY = tarY - wristSeg.setPosY;
	//Find total distance
	double dist = std::sqrt((dirX*dirX)+(dirY*dirY));
	//Speed stuff
	dist /= (0.06); //3 Inches per second, 0.02 each loop's time 50 Hz
	//Disance calculation
	if(dist == 0)
		{
		newTarX = wristSeg.setPosX;
		newTarY = wristSeg.setPosY;
		}
	else
		{
		newTarX = (dirX / dist) + wristSeg.setPosX;
		newTarY = (dirY / dist) + wristSeg.setPosY;
		}
	if((std::abs(wristSeg.setPosX - tarX) < 1))
		{
		newTarX = tarX;
		}
	if(std::abs(wristSeg.setPosY - tarY) < 1)
		newTarY = tarY;
...
bool ManipulatorArm::pickUpCube()
	...
	switch(pickUpCase)
		...
		case 1:
			counter1++;
			//Move shoulder down
			setShoulderAbsAngle(shStart - counter1);
			if(checkForCube())
				counter2++;
			if(counter2 == 5)
				{
		...
		case 2:
			...
			switch(elCase)
				...
				case 0:
					//Set Elbow speed
					setElbowSpeed(0.25);
					if((elbow->GetSensorCollection().GetPulseWidthVelocity() > 0.2)||(count > 40))
						elCase = 1;
					break;
				case 1:
					//Check speed
					if(std::abs(getElbowAngular() - (ELBOWLOWENCODER)) > 300) //Check if there may have been a chain jump
						...
						elChainJump = false;
					frc::SmartDashboard::PutBoolean("Elbow Skip", elChainJump);
					if (elbow->GetSensorCollection().GetPulseWidthVelocity() < 0.1)
						elCase = 2;
					break;
				case 2:
					//Angle is -159.2
				...
				case 3:
					if (count > 25)
						{
						elbow->SetSelectedSensorPosition(ELBOWLOWENCODER-75, 0, 0);
						elCase = 4;
						}
					break;
				}
